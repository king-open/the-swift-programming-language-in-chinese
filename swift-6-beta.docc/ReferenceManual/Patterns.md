...
标识符模式在值绑定模式中将新的命名变量或常量绑定到其匹配的值。
...
*枚举案例* 。枚举 case 模式出现在 `switch` 语句 case 标签以及 `if`  、 `while`  、 `guard` 和`for`  - `in` 语句的 case 条件中。
...
枚举案例模式还可以匹配包装在可选中的该案例值。这种简化的语法允许您省略可选模式。请注意，由于 `Optional` 是作为枚举实现的，因此 .none 和 .some 可以与枚举类型的案例在同一个 switch 语句中出现
...
可选模式与包含在 `Optional<Wrapped>` 枚举的 `some(Wrapped)` 情况中的值相匹配。可选模式由标识符模式紧跟问号组成，并出现在与枚举案例模式相同的位置
...
is 模式在运行时匹配一个值，如果该值的类型与 is 模式右侧指定的类型相同——或者是该类型的子类。 `is` 模式的行为类似于 `is` 运算符，因为它们都执行类型转换，但会丢弃返回的类型。

as 模式在运行时匹配一个值，如果该值的类型与 as 模式右侧指定的类型相同——或者是该类型的子类。如果匹配成功，则匹配值的类型将转换为 `as` 模式右侧指定的模式

以下是一个使用 `switch` 语句匹配包含关联值的枚举案例的示例，请参阅 <doc:TypeCasting#Type-Casting-for-Any-and-AnyObject>.
